...
**Spring.xml 父容器 component-scan 排除 controller**

**Spring-servlet.xml 子容器 component-scan 只包含 controller**  

子容器，只配置web相关的东西，这样如果有一天我要切换，方便管理，就怕重复加载bean
...
protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac, ServletContext sc) {
   ...
   //servlet上下文跟spring上下文联通
    ...
    if (configLocationParam != null) {
   	...
   	// getEnvironment跟getBean一样，get不到就create
...
上面调用createWebApplicationContext方法，走到这里面

### 一般来说就是加载了一个spring 的 XmlWebApplicationContext
...
/**
    ...
    // CONTEXT_CLASS_PARAM 现在过时了，就是一个配置
    ...
    // 没有就使用默认的
    // defaultStrategies这个东西在静态代码块初始化的时候就有了,会读取ContextLoader.properties
    // 这个配置文件是现在源码里面的
    // 其实就是XmlWebApplicationContext
		else {
...
### OnRefresh掉过来的，其实在初始化的时候Dispatcher就干这一件事，加载组件，剩下的都是他爹干了
